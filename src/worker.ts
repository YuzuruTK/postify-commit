  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" && url.pathname === "/linkedin/auth") {
      try {
        ...
        await env.LINKEDIN_KV.put(
          `${linkedinStorage.statePrefix}${state}`,
          "1",
          { expirationTtl: linkedinStorage.stateTtlSeconds },
        );
    ...
    if (request.method === "GET" && url.pathname === "/linkedin/callback") {
      try {
        ...
        return cors(json({
          connected: true,
          name: connection.user.name ?? null,
    ...
    if (request.method === "POST" && url.pathname === "/linkedin/publish") {
      try {
        ...
        if (typeof body.post !== "string" || !body.post.trim()) {
          return cors(json({ error: "post must be a non-empty string" }, 400));
        }
        ...
        const message = error instanceof Error ? error.message : "Unexpected error";
        const status = message.includes("must be") || message.includes("not connected") || message.includes("reconnect")
          ? 400
          : 502;
